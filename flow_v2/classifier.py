...
Analyzes audio.wav to determine which processing workflow to use.

Outputs:
  classification.json   — measured features + detected type
  .stepC.done           — sentinel to skip re-classification on resume

Usage:
  python -m flow_v2.classifier <output_dir>
"""
...
def classify(output_dir: Path, model_size: str = "small") -> VideoType:
    ...
    Checks .stepC.done sentinel first; returns cached result if present.
    """
    sentinel = output_dir / ".stepC.done"
    ...
    if sentinel.exists() and result_path.exists():
        ...
        print(f"[classify] Skip — cached result: {vtype.value}")
        return vtype
    ...
    print("[classify] Analyzing audio features …")
    ...
    if srt_path.exists():
        ...
        print(f"[classify] speech_ratio from SRT: {speech_ratio:.2f}")
    else:
        speech_ratio = _speech_ratio_from_whisper(wav_path, duration, model_size)
        print(f"[classify] speech_ratio from Whisper ({model_size}): {speech_ratio:.2f}")
...
def _measure_voice_energy_ratio(wav_path: Path) -> float:
    ...
    Note: background music bass can raise this, so it is used together with
    other features rather than alone.
    """
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m flow_v2.classifier <output_dir>")
        sys.exit(1)
    vt = classify(Path(sys.argv[1]))
